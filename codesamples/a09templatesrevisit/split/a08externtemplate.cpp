...
In C++, the extern template declaration is used to explicitly instantiate templates, 
which can improve compilation times and reduce code bloat.
 This feature allows you to separate the declaration of a template from its definition,
  ensuring that the template is instantiated in only one translation unit,
   while other translation units can use the pre-instantiated template.
